  ...
  return (
    <Link href={`/p?id=${p.id}`} className="group flex flex-col items-center text-center">
      <span className="relative block aspect-square w-full overflow-hidden rounded-full bg-elfia-blush ring-1 ring-elfia-line">
        ...
        {p.image_key ? (
          // eslint-disable-next-line @next/next/no-img-element
          /* `relative` is load-bearing: the backdrop above is positioned,
             and a positioned box paints over a static one whatever the
             source order — without it the photo would sit UNDER its own
             hover backdrop. */
